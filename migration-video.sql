...
-- Update api_type CHECK constraint to include 'google'
ALTER TABLE public.ai_agents DROP CONSTRAINT IF EXISTS ai_agents_api_type_check;
ALTER TABLE public.ai_agents ADD CONSTRAINT ai_agents_api_type_check CHECK (api_type IN ('openai', 'gemini', 'openrouter', 'custom', 'huggingface', 'google'));
