...
import {
  ...
  Send, CheckCircle2, Clock, XCircle, ChevronDown, Plus, ArrowLeft
} from 'lucide-react';
  ...
  if (!user) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <a href="/login" className="inline-flex items-center px-4 py-2 rounded-lg bg-indigo-500 hover:bg-indigo-400 text-white text-[13px] font-medium transition-colors">
          Sign In
        </a>
      </div>
    );
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {!showNew && !selectedTicket && (
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-2 rounded-lg bg-indigo-500 hover:bg-indigo-400 text-white text-[13px] font-medium transition-all hover:shadow-lg hover:shadow-indigo-500/20"
            >
          ...
          {showNew && (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowNew(false)} className="text-[13px] text-zinc-400 hover:text-zinc-200">
                  Cancel
                </button>
              </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                  {categories.map(cat => {
                    ...
                    return (
                      <button
                        key={cat.id}
                        onClick={() => setNewTicket(prev => ({ ...prev, category: cat.id }))}
                      ...
                      >
                        <div className={`w-8 h-8 rounded-lg ${cat.color} flex items-center justify-center mb-1.5`}>
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors capitalize ${
                        newTicket.priority === p
                          ? p === 'urgent' ? 'bg-red-500/15 text-red-300 border border-red-500/20'
                          : p === 'high' ? 'bg-orange-500/15 text-orange-300 border border-orange-500/20'
                          : 'bg-indigo-500/15 text-indigo-300 border border-indigo-500/20'
                          : 'text-zinc-400 border border-white/[0.05] hover:bg-white/[0.04]'
                      }`}
                    >
                      {p}
                    </button>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className="input-dark pl-3.5 text-[13px]"
                  maxLength={200}
                />
              </div>
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  className="input-dark pl-3.5 text-[13px] min-h-[120px] resize-y"
                  maxLength={2000}
                />
              </div>
              ...
              <button
                ...
                disabled={submitting}
                className="flex items-center space-x-1.5 px-4 py-2 rounded-lg bg-indigo-500 hover:bg-indigo-400 text-white text-[13px] font-medium transition-all disabled:opacity-50"
              >
                {submitting ? (
          ...
          {selectedTicket && !showNew && (
            ...
            >
              <button
                ...
                className="flex items-center space-x-1 text-[13px] text-zinc-400 hover:text-zinc-200 mb-4"
              >
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-medium border ${
                        statusConfig[selectedTicket.status]?.color || 'text-zinc-400 bg-zinc-500/10'
                      }`}>
                  ...
                  <div
                    ...
                    className={`p-3 rounded-xl ${
                      msg.is_admin
                        ...
                        : 'bg-white/[0.03] border border-white/[0.05] mr-4'
                    }`}
                  >
              ...
              {selectedTicket.status !== 'closed' && (
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    className="p-2 self-end rounded-lg bg-indigo-500 hover:bg-indigo-400 text-white disabled:opacity-30 transition-colors"
                  >
                  ...
                  {tickets.map(ticket => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left bg-white/[0.03] border border-white/[0.05] rounded-xl p-4 hover:bg-white/[0.04] hover:border-white/[0.08] transition-all"
                      >
                        <div className="flex items-start space-x-3">
                          <div className={`w-9 h-9 rounded-lg ${catConfig?.color || 'text-zinc-400 bg-zinc-500/10'} flex items-center justify-center flex-shrink-0`}>
                            <CatIcon className="w-4 h-4" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center space-x-2 mb-0.5">
                              <p className="text-[13px] font-medium truncate">{ticket.subject}</p>
                              <span className={`px-1.5 py-0.5 rounded text-[9px] font-medium border flex-shrink-0 ${status.color}`}>
                                {status.label}
                              </span>
                            </div>
                            ...
                            <div className="flex items-center space-x-2 mt-1">
                              ...
                              {ticket.priority !== 'normal' && (
                                <span className={`text-[10px] font-medium capitalize ${
                                  ...
                                  ticket.priority === 'high' ? 'text-orange-400' : 'text-zinc-500'
                                }`}>
                                  {ticket.priority}
                                </span>
                              )}
